...
@njit
def solve_model(t_max, rho1, rho2, S, x_min, x_max, n, D_s, D_b, chi, r, k, lambd, t_c, t_f, x_l, q, beta, dt_size, alpha, S_plus, S_minus, S_max):
    ...
    #for i in range(len(t)):
        ...
        for j in range(1,n-1):
            ...
            dSdt[j] = D_s*((S[j+1] - S[j])/dx**2 - (S[j]-S[j-1])/dx**2) - lambd*(S[j]/(S[j] + S_max))*rho1[j] + q_eff*dirac[j] - lambd*(S[j]/(S[j] + S_max))*rho2[j]
            # Chemotaxis
            ...
            chem = chi*(((rho1[j+1] - rho1[j-1])*(f_front - f_back)/(4*dx**2)) + rho1[j]*((f_front - 2*f_middle + f_back)/dx**2))
            # Growth
            growth1 = r*rho1[j]
            growth2 = r*rho2[j]
            # Competition
            ...
            death1 = lambd*beta*rho1[j]*(S[j]/(S[j] + S_max))
            death2 = lambd*beta*rho2[j]*(S[j]/(S[j] + S_max))
            # Bacterial equation
        ...
        rho1[np.where(rho1 < 1e-6)[0]] = 0
        rho2[np.where(rho2 < 1e-6)[0]] = 0
