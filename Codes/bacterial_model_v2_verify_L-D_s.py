...
print('\nReady to run simulations. Varying L and D_s \n')
...
for i in range(len(x_max)):
    for j in range(len(D_ss)):
        ...
        print(f"Run {i+1} of {len(x_max)} - L: {x_max[i]:.2f} - Total steps: {n}", end = "\r")
        ...
        # S[50] = 5
        rho = np.random.uniform(0.05, 0.1, n)
        rhos, Ss, tot_rho, tot_S, idx, dx, dt, x = solve_model(t_max, rho, S, 0, x_max[i], n, D_ss[j], D_b, chi, r, k, lambd, t_c, x_max[i]/2, q, beta, dt_size)
        rhos = np.array(rhos)
        Ss = np.array(Ss)
        if (rhos < 0).any() == False:
            ...
            tot_rho_profile[f"{x_max[i]:.2f} - {D_ss[j]:.2f}"] = np.nan

print("\nSimulations done! \n")

try:
    pd.DataFrame(final_profile_rho).to_csv('final_rho_profile_varying_L-D_s.csv')
    pd.DataFrame(final_profile_S).to_csv('final_S_profile_varying_L-D_s.csv')
    pd.DataFrame(tot_rho_profile).to_csv('temporal_N_profile_varying_L-D_s.csv')
except:
    print('Could not save files as DataFrames, saving using Pickle instead')
    with open('final_rho_profile_varying_L-D_s.pkl', "wb") as fp:
        pickle.dump(final_profile_rho, fp)
    with open('final_S_profile_varying_L-D_s.pkl', "wb") as fp:
        pickle.dump(final_profile_S, fp)
    with open('temporal_N_profile_varying_L-D_s.pkl', "wb") as fp:
